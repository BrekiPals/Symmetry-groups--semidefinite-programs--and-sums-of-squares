...
A polynomial mapping $f:\R^n\rightarrow \R^{n_i}$ with $f_i\in\R[\mathbf{x}]$ and 
...
By this change of coordinates we can assume that $\tilde{P}_i$ has a special form where each $r_i\times r_i$ subblock is a monomial multiple of the first principal $r_i\times r_i$ subblock $\Pi_i$. Then $\tilde{f}$ can be written as 
\[

...
\]
for a polynomial $f$ of degree $d$ in $n$ variables $\mathbf{x}=(x_1,\ldots,x_n)$.\\
...
If we further assume that $f\in \R[\mathbf{x}]^G$ is an invariant polynomial, then $A$ will be an invariant matrix and using the methods described in the previous section we can actually write it as 
...
\begin{enumerate}
    ...
    \[
        \tilde{f}(\theta_1,\theta_2)=\left<S_1,\Pi_1\right>+\left<S_5,\Pi_5\right>.
    \] 
\end{enumerate}

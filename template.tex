...
Furthermore The paper showed that with these constraints we can actually change coordinates so that all the matrices 
...
variables which is invariant with respect to a linear representation $\vartheta:G\rightarrow GL(\R^n),$
...
\[ \min_{X\in\cup\mathcal{L}\cap \mathcal{S}_+^N } \left<C,X\right> \rightarrow 
\min_{X\in\mathcal{F}\cup\mathcal{L}\cap \mathcal{S}_+^N } \left<C,X\right> \rightarrow

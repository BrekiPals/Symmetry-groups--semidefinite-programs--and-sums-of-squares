...
\]
which gives us a lower bound for $g^\ast$.\\

The methods we present exploit symmetries in polynomials and semidefinite programming (SDP) in order to get a reduction in the problem size for this latter problem. We want to do this for two reasons: Firstly, we get a faster solution since the time complexity of SDP has been shown to be polynomial \todo{do we have to cite this?}. Secondly, smaller problem size give more accurate solutions may that be due to numerical conditioning or numerical errors.\\
...
At first, we give a closer look into the ideas that make everything work. We do this by first focussing on a general symmetric SDP. Then we consider more closely the problem of representing a polynomial as a sum of squares.
Next, we present a concrete algorithm to solve for $f^{SOS}$ and obtain a certificate of the non-negativity of $f(x)-f^{SOS}$, complete with an example. Lastly, we summarize the possible computational savings.\\%TODO: Check if this is still up to date when we print.
...
<<<<<<< HEAD
We will now look at some invariant theory in order to simplify the problem even further. 
...
>>>>>>> 9d647425ae922b9be8e9c0c31c071778531f6330
...
isomorphic to a polynomial ring and the secondary invariants are not needed. %TODO: This sentence should go somewhere else, but I haven't figured out where.

Next we want to exploit the algebraic structure of the module of equivariants, that is for each irreducible representation $\vartheta_i$, tha module $M_i$ of $\vartheta$-$\vartheta_i$-equivariants is a free module over the ring in the primary invariants:
